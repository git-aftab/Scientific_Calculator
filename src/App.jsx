...
export default function App() {
  ...
  const internalInput = useMemo(() => operations.map(op => op.value).join(''), [operations]);
  const displayInput = useMemo(() => operations.map(op => op.display).join(''), [operations]);
  ...
  const handleButtonClick = useCallback((value, type, display) => {
    switch (value) {
      case 'ALL_CLEAR':
        setOperations([]);
        setResult('');
      ...
      case 'DEG':
        ...
        break;
      default:
        setOperations(prev => [...prev, { value, display: display || value }]);
        break;
    }
  }, [calculateResult]);
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
        ...
        if (buttonToClick) {
            handleButtonClick(buttonToClick.value, buttonToClick.type, buttonToClick.display);
        }
    };
      ...
      <div className="w-full max-w-sm md:max-w-md lg:max-w-lg mx-auto p-4">
        ...
        <div className="bg-black/30 backdrop-blur-sm rounded-2xl shadow-2xl shadow-cyan-500/10 overflow-hidden border border-white/10">
          ...
          <div className="p-6 text-white text-right">
            ...
            <div className="h-10 text-xl font-light truncate" style={{ direction: 'ltr' }}>{displayInput || '0'}</div>
            <div className="h-16 text-5xl font-bold truncate">{result}</div>
            <div className="h-4 text-xs text-cyan-400 font-mono tracking-widest">{isRadians ? 'RAD' : 'DEG'}</div>
          </div>
          ...
          <div className="grid grid-cols-5 gap-2 p-4 bg-black/20">
            {allButtons.map((btn) => {
              ...
              return (
                <button
                  key={btn.label}
                  onClick={() => handleButtonClick(btn.value, btn.type, btn.display)}
                  className={`
